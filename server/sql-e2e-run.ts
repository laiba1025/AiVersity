...
(async () => {
  try {
    ...
    const programs = await get(host, port, '/api/programs');
    const courses = await get(host, port, '/api/courses?program=' + encodeURIComponent('CS BSc'));

    console.log(JSON.stringify({ ok: true, host, port, health, programs, coursesCount: Array.isArray(courses) ? courses.length : undefined }, null, 2));

    httpServer.close();
    process.exit(0);
  } catch (err) {
    console.error('SQL E2E run failed:', err);
    process.exit(1);
  }
})();
